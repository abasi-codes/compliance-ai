        ...
        <CardContent>
          <div className="space-y-6">
            ...
            <div className="p-4 bg-gradient-to-br from-primary-50 to-accent-50 rounded-xl border border-primary-100 flex items-center gap-4">
              <span className="text-sm font-medium text-neutral-600">Overall Maturity:</span>
              <span className="text-4xl font-bold gradient-text">
                {content.executive_summary.overall_maturity.toFixed(1)}
              </span>
              <span className="text-lg text-neutral-400">/4.0</span>
            </div>

            {/* Key Strengths */}
            <div>
              <h4 className="text-sm font-semibold text-neutral-700 mb-3">Key Strengths</h4>
            ...
            {/* Critical Gaps */}
            <div>
              <h4 className="text-sm font-semibold text-neutral-700 mb-3">Critical Gaps</h4>
              <ul className="space-y-2">
            ...
            {content.maturity_summary.by_function.map((func, index) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-neutral-700">{func.name}</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-32 bg-neutral-200 rounded-full h-2.5 overflow-hidden">
                    <div
                  ...
                  <span className="text-sm font-semibold text-neutral-700 w-8">{func.score.toFixed(1)}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <CardContent>
          <div className="grid grid-cols-4 gap-4">
            ...
            {Object.entries(content.deviations.by_severity).map(([severity, count]) => {
              ...
              const style = config[severity] || { bg: 'from-slate-50 to-slate-100', border: 'border-neutral-200', text: 'text-neutral-700' };
              ...
              return (
                ...
                >
                  ...
                  <p className="text-sm text-neutral-500 mt-1 capitalize">{severity.toLowerCase()}</p>
                </div>
              );
            })}
          </div>
                ...
                <div className="space-y-3">
                  {content.recommendations.immediate.map((rec, i) => (
                    <div key={i} className="pl-4 border-l-2 border-red-200 py-2">
                      <p className="font-medium text-neutral-900">{rec.title}</p>
                      <p className="text-sm text-neutral-500 mt-1">{rec.description}</p>
                    </div>
                  ))}
                </div>
              </div>

  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl border border-slate-200">
        ...
        <p className="text-sm text-slate-600 leading-relaxed">{explanation.rationale}</p>
      </div>
            ...
            {explanation.components.map((component, index) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="font-medium text-slate-900">{component.type}</span>
                  {component.source && (
          ...
          <div className="space-y-2">
            {explanation.evidence_citations.map((citation, index) => (
              ...
              >
                ...
                <span className="text-slate-600 ml-1">{citation.text}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {explanation.confidence_factors && Object.keys(explanation.confidence_factors).length > 0 && (
        <div>
          <h4 className="text-sm font-semibold text-slate-700 mb-3">Confidence Factors</h4>
